...
class MediaGateway:
    ...
    def record_video_heartbeat(
        ...
        *,
        ...
        status.video.updated_at = utc_now()
        status.video.last_heartbeat_at = status.video.updated_at
    ...
    def record_video_sample(
        ...
        *,
        ...
        now = utc_now()
        status.video.updated_at = now
        status.video.first_frame_at = status.video.first_frame_at or now
        status.video.last_frame_at = now
        self._record_video_timing(session_id=session_id, video=status.video)
        status.updated_at = status.video.updated_at
        if is_keyframe or status.video.frame_count % 30 == 1:
            self._events.add(
                ...
                "video_sample",
                {
                    ...
                    "estimated_fps": status.video.estimated_fps,
