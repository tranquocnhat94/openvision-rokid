...
runtime. It only exposes configuration status plus snapshot/live-stream ingress
contracts that a separate OpenVision/Rokid detector process can call.
"""
...
VALID_MODES = {"disabled", "external_snapshot", "external_stream"}
...
class Yolo26AdapterStatus:
    ...
    mode: str
    status: str
    snapshot_ingest_enabled: bool
    stream_ingest_enabled: bool
...
class Yolo26RokidAdapter:
    ...
    def validate_external_snapshot(self, *, source: str) -> dict[str, Any]:
        return self._validate_external_ingress(source=source, expected_mode="external_snapshot", ingress_kind="snapshot")

    def validate_external_stream(self, *, source: str) -> dict[str, Any]:
        return self._validate_external_ingress(source=source, expected_mode="external_stream", ingress_kind="stream")

    def _validate_external_ingress(self, *, source: str, expected_mode: str, ingress_kind: str) -> dict[str, Any]:
        settings = load_yolo26_adapter_settings()
        ...
        if status.status == "disabled":
            ...
            return {
                ...
                "error": {
                    ...
                    "message": f"Set OPENVISION_YOLO26_MODE={expected_mode} before posting YOLO26 {ingress_kind} detections.",
                },
            }
        ...
        if settings.mode != expected_mode:
            ...
            return {
                ...
                "error": {
                    ...
                    "message": f"OPENVISION_YOLO26_MODE is {settings.mode}; set it to {expected_mode} for {ingress_kind} ingress.",
                },
            }
        source_error = _validate_external_source(clean_source, ingress_kind=ingress_kind)
        if source_error:
            self._events.add(
                "adapter.yolo26",
                f"{ingress_kind}_rejected",
        ...
        source_prefix = "yolo26_rokid_stream" if ingress_kind == "stream" else "yolo26_rokid"
        return {
            "status": "accepted",
            "source": f"{source_prefix}:{clean_source}",
...
def _build_status(settings: Yolo26AdapterSettings) -> Yolo26AdapterStatus:
    ...
    if settings.mode not in VALID_MODES:
    ...
    elif unsafe_path := _unsafe_runtime_path(settings):
        status = "invalid"
        message = (
            f"Configured YOLO26 {unsafe_path['field']} path appears to reference a protected "
            f"{unsafe_path['marker']} runtime; use a separate OpenVision/Rokid runtime path."
        )
        ...
        status = "ready"
        message = "Ready to accept live bbox frames from a separate OpenVision/Rokid YOLO26 stream worker."
    else:
        status = "invalid"
        message = "YOLO26 v2 only supports disabled, external_snapshot, or external_stream mode; it never starts or binds detector processes."

    return Yolo26AdapterStatus(
        name="yolo26_rokid",
        mode=settings.mode,
        status=status,
        snapshot_ingest_enabled=status == "ready" and settings.mode == "external_snapshot",
        stream_ingest_enabled=status == "ready" and settings.mode == "external_stream",
...
def _unsafe_runtime_path(settings: Yolo26AdapterSettings) -> dict[str, str] | None:
    for field, path in (("engine", settings.engine_path), ("labels", settings.labels_path)):
        ...
        for marker in FORBIDDEN_SOURCE_MARKERS:
            if marker in lowered:
                return {"field": field, "marker": marker}
    return None
...
def _validate_external_source(source: str, *, ingress_kind: str) -> dict[str, str] | None:
    if not source:
        return {
            "code": f"missing_{ingress_kind}_source",
            "message": f"YOLO26 {ingress_kind} source is required and must identify the separate Rokid/OpenVision runtime.",
        }
    if any(marker in source for marker in FORBIDDEN_SOURCE_MARKERS):
        return {
            "code": f"forbidden_{ingress_kind}_source",
            "message": f"YOLO26 {ingress_kind} detections from Ring/security runtimes are not accepted by OpenVision v2.",
        }
    if not any(marker in source for marker in ALLOWED_SOURCE_MARKERS):
        return {
            "code": f"invalid_{ingress_kind}_source",
            "message": f"YOLO26 {ingress_kind} source must identify a separate Rokid/OpenVision runtime.",
        }
    return None
