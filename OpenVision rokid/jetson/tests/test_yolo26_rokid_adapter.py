...
class Yolo26RokidAdapterTest(unittest.TestCase):
    ...
    def test_external_snapshot_mode_is_ready_without_ring_runtime(self):
        ...
        self.assertTrue(status["snapshot_ingest_enabled"])
        self.assertFalse(status["stream_ingest_enabled"])
        self.assertFalse(status["engine_exists"])
        self.assertFalse(status["labels_exists"])

    def test_external_stream_mode_is_ready_without_ring_runtime(self):
        with patch.dict(os.environ, {"OPENVISION_YOLO26_MODE": "external_stream"}, clear=True):
            adapter = Yolo26RokidAdapter(events=InMemoryEventStore())
            status = adapter.status()

        self.assertEqual(status["status"], "ready")
        self.assertFalse(status["snapshot_ingest_enabled"])
    ...
    def test_external_stream_accepts_openvision_runtime_source(self):
        with patch.dict(os.environ, {"OPENVISION_YOLO26_MODE": "external_stream"}, clear=True):
            adapter = Yolo26RokidAdapter(events=InMemoryEventStore())
            result = adapter.validate_external_stream(source="openvision_iphone_yolo26")

        self.assertEqual(result["status"], "accepted")
        self.assertEqual(result["source"], "yolo26_rokid_stream:openvision_iphone_yolo26")

    def test_external_stream_does_not_accept_snapshot_ingress(self):
        with patch.dict(os.environ, {"OPENVISION_YOLO26_MODE": "external_stream"}, clear=True):
            adapter = Yolo26RokidAdapter(events=InMemoryEventStore())
            result = adapter.validate_external_snapshot(source="openvision_iphone_yolo26")
    ...
    def test_external_stream_rejects_ring_sources(self):
        events = InMemoryEventStore()
        with patch.dict(os.environ, {"OPENVISION_YOLO26_MODE": "external_stream"}, clear=True):
            adapter = Yolo26RokidAdapter(events=events)
            ring = adapter.validate_external_stream(source="ring_security_yolo26")

        self.assertEqual(ring["status"], "error")
        self.assertEqual(ring["error"]["code"], "forbidden_stream_source")
        self.assertEqual(events.list()[-1]["payload"]["reason"], "forbidden_stream_source")

    def test_external_stream_rejects_protected_runtime_paths(self):
        with patch.dict(
            os.environ,
            {
                "OPENVISION_YOLO26_MODE": "external_stream",
                "OPENVISION_YOLO26_ENGINE_PATH": "/opt/ring/yolo26/security.engine",
            },
            clear=True,
        ):
            adapter = Yolo26RokidAdapter(events=InMemoryEventStore())
        ...
        self.assertIn("protected", status["message"])
