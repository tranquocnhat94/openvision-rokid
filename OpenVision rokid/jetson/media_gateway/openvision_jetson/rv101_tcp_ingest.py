...
from .rv101_video_metadata import video_metadata_from_header
...
VideoH264Handler = Callable[[str, dict[str, Any], bytes, dict[str, Any]], Any]
VideoCloseHandler = Callable[[str], None]
VideoFrameAllowed = Callable[[str, dict[str, Any], int], bool]
VideoFrameRecorder = Callable[[str, dict[str, Any], bytes, int], Any]
AudioFrameRecorder = Callable[[str, dict[str, Any], bytes, int], Any]
...
class Rv101TcpIngestService:
    def __init__(
        ...
        *,
        ...
        video_h264_handler: VideoH264Handler | None = None,
        video_close_handler: VideoCloseHandler | None = None,
        video_frame_allowed: VideoFrameAllowed | None = None,
        video_frame_recorder: VideoFrameRecorder | None = None,
        audio_frame_recorder: AudioFrameRecorder | None = None,
    ) -> None:
        self._media = media
        self._events = events
        ...
        self._video_h264_handler = video_h264_handler
        self._video_close_handler = video_close_handler
        self._video_frame_allowed = video_frame_allowed
        self._video_frame_recorder = video_frame_recorder
        self._audio_frame_recorder = audio_frame_recorder
        ...
        self._video_rejected_sessions: set[str] = set()
    ...
    async def _handle_stream(
        ...
        seen_video_sessions: set[str] = set()
        ...
        try:
            while True:
                ...
                if kind == "video":
                    seen_video_sessions.add(_session_id(frame.header))
            ...
            if kind == "video" and self._video_close_handler:
                for session_id in sorted(seen_video_sessions):
                    self._video_close_handler(session_id)
    ...
    def _handle_video_frame(self, header: dict[str, Any], payload: bytes, message_type: int) -> None:
        ...
        if self._video_frame_allowed and not self._video_frame_allowed(session_id, header, message_type):
            if session_id not in self._video_rejected_sessions:
                self._events.add(
                    "rv101_ingest",
                    "video_frame_ignored",
                    {
                        "reason": "inactive_live_video",
                ...
                self._video_rejected_sessions.add(session_id)
            return
        self._video_rejected_sessions.discard(session_id)
        if self._video_frame_recorder:
            self._video_frame_recorder(session_id, header, payload, message_type)
        metadata = video_metadata_from_header(header)
        ...
        if message_type != TYPE_VIDEO_SAMPLE:
            return
        status = self._media.record_video_sample(
            session_id=session_id,
            transport="rv101_tcp",
            codec="video/avc",
        ...
        if self._video_h264_handler:
            self._video_h264_handler(
                session_id=session_id,
                header=header,
                payload=payload,
                media_status=status,
    ...
    async def _handle_audio_frame(self, header: dict[str, Any], payload: bytes, message_type: int) -> None:
        session_id = _session_id(header)
        if self._audio_frame_recorder:
            self._audio_frame_recorder(session_id, header, payload, message_type)
