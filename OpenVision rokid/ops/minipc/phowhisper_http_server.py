...
class PhoWhisperHandler(BaseHTTPRequestHandler):
    ...
    def _is_authorized(self) -> bool:
        ...
        if not expected:
            return True
        token = (
            str(self.headers.get("X-OpenVision-Debug-STT-Token") or "").strip()
            or str(self.headers.get("X-Rokid-Debug-STT-Token") or "").strip()
        )
...
class PhoWhisperHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        *,
        ...
        self.worker.auth_required = bool(auth_token)
...
def _load_auth_token(args: argparse.Namespace) -> str:
    direct = str(args.auth_token or os.getenv("OPENVISION_PHOWHISPER_AUTH_TOKEN") or "").strip()
    if direct:
        return direct
    token_file = str(args.auth_token_file or os.getenv("OPENVISION_PHOWHISPER_AUTH_TOKEN_FILE") or "").strip()
    if not token_file:
        return ""
